...
my $SINGLELINE_RULE_REGEXP = '^\s*(?:alert|drop|log|pass|reject|sdrop|activate|dynamic)\s.+msg\s*:\s*"'.
                             '(.+?)"\s*;.*sid\s*:\s*(\d+)\s*;.*\)\s*$'; # ';

# Regexp to match the start (the first line) of a possible multi-line rule.
my $MULTILINE_RULE_REGEXP  = '^\s*(?:alert|drop|log|pass|reject|sdrop|activate|dynamic)\s.*\\\\\s*\n$'; # ';
